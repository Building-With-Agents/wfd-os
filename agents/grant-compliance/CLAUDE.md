...
### QuickBooks is read-only — enforced in our HTTP client

**What we cannot control (the inevitable part):**

QuickBooks Online's OAuth2 flow uses a single scope — `com.intuit.quickbooks.accounting`
— which grants **both read AND write access** to accounting data. Intuit
does not offer a read-only scope. The token we receive from the OAuth
exchange is, as far as Intuit is concerned, capable of POSTing journal
entries, modifying vendors, deleting bills, etc.

Additionally, Intuit restricts OAuth app authorization to **Master Admin
or Company Admin** users of the target QB company. The authorizing user's
role does NOT narrow the token's capabilities. Trying to enforce "read
only" by having a low-privilege user authorize the flow does not work —
the user can't authorize at all, and if they could, the token would still
carry full read+write scope.

**What we actually enforce (three real layers, clearly ranked):**

**Layer 1 — OAuth scope (out of our control).** The token can write. We
treat this as a fact of life.

**Layer 2 — `_ReadOnlyHttpxClient` (primary enforcement, ours).**
...
This is the **only layer that actually prevents writes.** If this is
bypassed, the token will happily write.

Mechanism:
- `_ReadOnlyHttpxClient.request()` raises `NotImplementedError` for any
  non-GET method (POST, PUT, PATCH, DELETE, HEAD, OPTIONS).
...
**Layer 3 — Intuit's audit log (post-hoc detection, not prevention).**
Every request QB receives is logged on Intuit's side with the app, user,
timestamp, endpoint, and verb. If Layer 2 is ever bypassed and a write
reaches QB, Intuit's audit log lets us detect it after the fact and
attribute it to this app. This is detection-only. It does not stop a
write in flight.
...
- (b) A separate `qb_writeback` module — do not add write methods to
  `client.py`. Writeback lives outside this file so Layer 2 stays clean
  for read.
- (c) A **distinct** OAuth credential for writes. This is audit separation
  on the Intuit side (different app or different authorizing admin → the
  writes are attributable to the writeback pathway specifically, not
  confused with read traffic). It does NOT limit what the write credential
  can do — Intuit's scope is still read+write. The separation is about
  traceability, not capability.
- (d) Its own approval gate and audit log entries on OUR side for every
  write (the append-only `audit_log` table), so we have a record
  independent of Intuit's.
