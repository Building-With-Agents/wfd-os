"""Scoping pipeline dataclasses.

Migrated from agents/scoping/models.py in #21. The old path is a re-export
shim for one deprecation cycle, and moving the source here breaks the
sharepoint -> scoping circular import that blocked #17's cleanup.

Kept as dataclasses (not Pydantic) to preserve exact construction
semantics: Apollo-webhook parsing in `ScopingRequest.from_webhook()` passes
empty strings for several fields, which Pydantic's default validation
would reject as missing-required. A later PR can convert to Pydantic
BaseModel once every call site is proven tolerant.
"""
