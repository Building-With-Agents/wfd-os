...
import {
  ...
  ChevronRight, Github, Star, Clock, DollarSign, Send, CheckCircle2, Copy, Sparkles,
} from "lucide-react"
...
function IntakeForm() {
  ...
  const copyReference = () => {
    if (referenceNumber && typeof navigator !== "undefined") {
  ...
  if (submitted) {
    const firstName = (formData.contact_name || "").trim().split(/\s+/)[0] || "there"
    return (
      <section id="intake-form" className="bg-muted/30 px-4 py-16">
        <div className="mx-auto max-w-md rounded-xl border bg-card p-8 text-center shadow-sm">
          ...
          <h2 className="text-2xl font-bold text-foreground">Thank you, {firstName}!</h2>
    ...
    <footer className="border-t border-border bg-card py-8">
      <div className="mx-auto max-w-6xl px-4 text-center">
        ...
        <p className="mt-2 text-xs text-muted-foreground">
          computingforall.org | info@computingforall.org | 501(c)(3) nonprofit
        </p>
      </div>
    </footer>
  )
...
export default function ForEmployersPage() {
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Conversation CTA — primary entry point */}
      <section className="px-4 py-16">
        <div className="mx-auto max-w-2xl">
          <a href="/cfa/ai-consulting/chat" className="block">
            <div className="group overflow-hidden rounded-2xl bg-gradient-to-br from-primary via-purple-600 to-purple-800 p-8 text-center shadow-lg transition-all hover:shadow-xl hover:scale-[1.01]">
              <Sparkles className="mx-auto mb-4 h-10 w-10 text-white/80" />
              <h2 className="text-2xl font-bold text-white sm:text-3xl">
                Tell us about your project
              </h2>
              <p className="mx-auto mt-3 max-w-md text-sm text-white/80">
                Have a conversation with our AI advisor. No forms, no commitment.
                Just tell us what you&apos;re trying to solve.
              </p>
              <div className="mt-6 inline-flex items-center gap-2 rounded-full bg-white px-6 py-2.5 text-sm font-semibold text-primary transition-colors group-hover:bg-white/90">
