...
import { Search, MapPin, GraduationCap, Briefcase, Star, Filter, ChevronDown, Compass, ExternalLink, ArrowRight } from "lucide-react"
...
export default function TalentShowcase() {
  ...
  const [filterSkills, setFilterSkills] = useState<FilterOption[]>([])
  const [filterLocations, setFilterLocations] = useState<{ label: string; count: number }[]>([])
  ...
  const loadCandidates = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/showcase/candidates?${params}`)
  ...
  const loadFilters = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/showcase/filters`)
      const data = await res.json()
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* CFA AI Consulting Bridge */}
      <section className="border-t border-border bg-gradient-to-r from-primary/5 via-primary/10 to-primary/5 px-4 py-10">
        <div className="mx-auto max-w-4xl">
          <div className="flex flex-col items-center gap-4 text-center sm:flex-row sm:text-left">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-foreground">Need more than just talent?</h3>
              <p className="text-sm text-muted-foreground">
                CFA builds complete agentic AI systems for coalition employers &mdash; using apprentices from this talent pool.
              </p>
            </div>
            <a href="/cfa/ai-consulting" className="flex-shrink-0">
              <Button variant="outline" className="gap-1">
                Learn about AI consulting <ArrowRight className="h-3.5 w-3.5" />
              </Button>
            </a>
          </div>
        </div>
